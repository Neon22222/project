...
export async function GET(request: NextRequest) {
  try {
    ...
    // For demo purposes, we'll assume half of the commissions are pending
    const pendingCommission = totalCommission / 2
    ...
    const referralHistory = userData.transactions
      ...
      .map((tx: any) => {
        ...
        const referralUser = userData.downlines.find((ref: any) => 
          ref.id === tx.userId || tx.createdAt >= ref.createdAt
        )
        ...
        return {
          ...
          username: referralUser?.username || 'Unknown User',
